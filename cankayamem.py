...
# ============================================
# ============================================
# KARANLIK BİLİMSEL TEMA (SON - KESİN)
# ============================================
st.markdown("""
<style>
...
if df.empty:
    st.error("⚠️ Veri bulunamadı! Lütfen Supabase bağlantısını kontrol edin.")
    st.stop()
...
with st.sidebar:
    ...
    filtered_df = filtered_df[(filtered_df['derslik_sayisi'] >= derslik_range[0]) & (filtered_df['derslik_sayisi'] <= derslik_range[1])]
...
<div class="main-header">
    ...
    <h2 style="font-size: 1.5rem; color: #9fa8da; position: relative; z-index: 1;">Norm Fazlası Öğretmen Dağılım ve Takip Sistemi</h2>
    <p style="color: #7986cb; position: relative; z-index: 1; margin-top: 0.5rem;">Son Güncelleme: {datetime.now().strftime('%d.%m.%Y %H:%M')} | Toplam Okul: {df['okul_id'].nunique()} | Toplam Branş: {df['brans_adi'].nunique()}</p>
</div>
""", unsafe_allow_html=True)
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    "📊 GENEL BAKIŞ", "🏫 OKUL ANALİZİ", "👨‍🏫 BRANŞ ANALİZİ",
    "⚠️ NORM FAZLASI", "📈 RAPORLAR", "🔗 OKUL LİNKLERİ", "🔄 GÖREVLENDİRME"
])
...
with tab1:
    ...
    with col1:
        st.markdown(f"""<div class="stat-card"><div class="stat-value">{filtered_df['okul_id'].nunique()}</div><div class="stat-label">🏫 Toplam Okul</div></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div class="stat-card"><div class="stat-value">{filtered_df['mevcut_ogretmen'].sum():,}</div><div class="stat-label">👨‍🏫 Toplam Öğretmen</div></div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div class="stat-card"><div class="stat-value">{filtered_df['norm_sayisi'].sum():,}</div><div class="stat-label">📋 Toplam Norm Kadro</div></div>""", unsafe_allow_html=True)
    with col4:
        st.markdown(f"""<div class="stat-card"><div class="stat-value" style="color: #ff5252;">{filtered_df['norm_fazlasi_sayisi'].sum()}</div><div class="stat-label">🔴 Norm Fazlası</div></div>""", unsafe_allow_html=True)
    with col5:
        st.markdown(f"""<div class="stat-card"><div class="stat-value" style="color: #69f0ae;">{filtered_df['norm_eksigi_sayisi'].sum()}</div><div class="stat-label">🟢 Norm Eksiği</div></div>""", unsafe_allow_html=True)
    ...
    with col1:
        ...
        fig = px.pie(type_dist, values='Okul Sayısı', names='Okul Türü', title='Okul Türlerine Göre Dağılım', color_discrete_sequence=px.colors.sequential.Plasma_r, hole=0.4)
        fig.update_traces(textposition='inside', textinfo='percent+label', textfont=dict(color='white', size=11))
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#e8eaf6'), title_font=dict(color='#e8eaf6', size=16), legend=dict(font=dict(color='#9fa8da')), height=450)
    ...
    with col2:
        ...
        fig = px.bar(norm_dist, x='Norm Durumu', y='Sayı', title='Norm Durumu Dağılımı', color='Norm Durumu', color_discrete_map={'Norm Fazlası': '#ff5252', 'Normal': '#ffab40', 'Norm Eksiği': '#69f0ae'}, text='Sayı')
        fig.update_traces(textposition='outside', textfont=dict(color='white', size=14))
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#e8eaf6'), title_font=dict(color='#e8eaf6', size=16), showlegend=False, height=450)
        st.plotly_chart(fig, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    
    st.markdown("### Okul Türüne Göre Norm Kadro Karşılaştırması")
    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
    type_analysis = filtered_df.groupby('okul_turu').agg({'norm_sayisi': 'sum', 'mevcut_ogretmen': 'sum', 'norm_fazlasi_sayisi': 'sum', 'norm_eksigi_sayisi': 'sum'}).reset_index()
    fig = go.Figure()
    fig.add_trace(go.Bar(name='Norm Kadro', x=type_analysis['okul_turu'], y=type_analysis['norm_sayisi'], marker_color='#82b1ff', marker_line_color='#448aff', marker_line_width=1.5))
    fig.add_trace(go.Bar(name='Mevcut Öğretmen', x=type_analysis['okul_turu'], y=type_analysis['mevcut_ogretmen'], marker_color='#b388ff', marker_line_color='#7c4dff', marker_line_width=1.5))
    fig.add_trace(go.Bar(name='Norm Fazlası', x=type_analysis['okul_turu'], y=type_analysis['norm_fazlasi_sayisi'], marker_color='#ff5252', marker_line_color='#d32f2f', marker_line_width=1.5))
    fig.add_trace(go.Bar(name='Norm Eksiği', x=type_analysis['okul_turu'], y=type_analysis['norm_eksigi_sayisi'], marker_color='#69f0ae', marker_line_color='#00c853', marker_line_width=1.5))
    fig.update_layout(barmode='group', title='Okul Türüne Göre Norm ve Mevcut Öğretmen Karşılaştırması', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#e8eaf6'), title_font=dict(color='#e8eaf6', size=16), legend=dict(font=dict(color='#9fa8da')), xaxis=dict(tickangle=-45, gridcolor='rgba(255,255,255,0.05)'), yaxis=dict(gridcolor='rgba(255,255,255,0.05)'), height=500)
    st.plotly_chart(fig, use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if schools_list:
        ...
        if selected_school:
            ...
            with col4:
                st.info(f"**👨‍🏫 Toplam Öğretmen:** {school_data['mevcut_ogretmen'].sum()}")
            ...
            display_df = school_data[['brans_adi', 'norm_sayisi', 'mevcut_ogretmen', 'norm_durumu', 'norm_fazlasi_sayisi', 'norm_eksigi_sayisi', 'ortalama_hizmet_puani']].copy()
            ...
            st.markdown('<div class="chart-container">', unsafe_allow_html=True)
            fig = go.Figure()
            fig.add_trace(go.Bar(name='Norm Kadro', x=school_data['brans_adi'], y=school_data['norm_sayisi'], marker_color='#82b1ff', marker_line_color='#448aff', marker_line_width=1.5, text=school_data['norm_sayisi'], textposition='outside', textfont=dict(color='#82b1ff', size=11)))
            fig.add_trace(go.Bar(name='Mevcut Öğretmen', x=school_data['brans_adi'], y=school_data['mevcut_ogretmen'], marker_color='#b388ff', marker_line_color='#7c4dff', marker_line_width=1.5, text=school_data['mevcut_ogretmen'], textposition='outside', textfont=dict(color='#b388ff', size=11)))
            fig.update_layout(title=f'{selected_school} - Branş Bazında Karşılaştırma', barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#e8eaf6'), title_font=dict(color='#e8eaf6', size=16), legend=dict(font=dict(color='#9fa8da')), xaxis=dict(tickangle=-45, gridcolor='rgba(255,255,255,0.05)'), yaxis=dict(gridcolor='rgba(255,255,255,0.05)'), height=450)
            st.plotly_chart(fig, use_container_width=True)
            st.markdown('</div>', unsafe_allow_html=True)
    else:
        st.warning("⚠️ Seçili filtrecolor: #e8eaf6; text-align: center; padding: 2rem; border-radius: 15px;
margin-bottom: 2rem; box-shadow: 0 8px 32px rgba(26, 35, 126, 0.3);
border: 1px solid rgba(255, 255, 255, 0.1); position: relative; overflow: hidden;
}
.main-header::before {
